...
gulp.task('build-ngdocs', [], function() {
    ...
    return gulp.src('web/js/mango-3.5/**/*.js')
        .pipe(gulpDocs.process(options))
        .pipe(gulp.dest('docs/mango-3.5'));
});
...
gulp.task('copy-docs', ['build-ngdocs'], function() {
    ...
    return gulp.src(['docs/mango-3.5/partials/api/*.html','docs/mango-3.5/js/docs-setup.js'])
        .pipe(gulp.dest('web/mdAdmin/views/docs'));
});

gulp.task('watchDocs', function() {
    // Watch .js files
    gulp.watch('web/js/mango-3.5/**/*.js', ['copy-docs']);
});
